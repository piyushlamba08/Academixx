...
def run_generator(chunk: str, to_generate: int, api_key: str, name: str) -> List[dict]:
    ...
    prompt = PromptTemplate(
        template=(
            ...
            "- ALWAYS calculate the answer mathematically step by step before writing options. Never guess.\n"
            "- If the answer is a fraction like 10 10/107%, convert it to decimal rounded to 2 places (e.g. 10.71%) and use that consistently in both options and answer field.\n"
            "- All 4 options MUST be realistic and numerically close to each other. For percentage questions, options should differ by small amounts like 1-5%. NEVER generate absurd values like 1057% or 2847%.\n"
            "- Options should be plausible distractors — close to the correct answer but clearly wrong.\n"
            ...
            "- Never use undefined, null, N/A, or an option label as the answer.\n\n"
            "{format_instructions}\n\n"
            "Practice Questions:\n{chunk}"
        ),
